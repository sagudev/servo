...
pub unsafe fn cross_origin_get(
    ...
    if is_generic_descriptor(&descriptor) {
...
fn is_accessor_descriptor(d: &PropertyDescriptor) -> bool {
    ...
    return !is_data && (d.hasSetter_() || d.hasGetter_());
}
...
fn is_generic_descriptor(d: &PropertyDescriptor) -> bool {
    !is_accessor_descriptor(d) && !is_data_descriptor(d)
}
